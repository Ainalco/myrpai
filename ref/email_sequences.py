...
async def add_sequence_email(
    ...
    config = db.query(models.EmailSequenceConfig).join(models.Workflow).filter(
...
async def update_sequence_email(
    ...
    email = db.query(models.SequenceEmail).join(
        models.EmailSequenceConfig
    ).join(models.Workflow).filter(
    ...
    if not email:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Sequence email not found"
        )
...
async def generate_emails_with_ai(
    ...
    """
    from ai_service import generate_sequence_emails
...
async def optimize_timing_with_ai(
    ...
    """
    from ai_service import optimize_email_timing
